...
// TeacherReportService handles the export logic
type TeacherReportService struct {
	repo repositories.ReportRepository
}

// NewTeacherReportService creates a new instance
func NewTeacherReportService(repo repositories.ReportRepository) *TeacherReportService {
	return &TeacherReportService{repo: repo}
}

// ExportToCSV writes the schedule data to the provided writer
func (s *TeacherReportService) ExportToCSV(
	...
	// 1. Fetch sorted data
	rows, err := s.repo.GetWeeklyTeacherReport(ctx, semesterID, version)
	if err != nil {
		return err
	}
	// 2. Get max week day
	...
	for i := range rows {
		...
		if currentTeacherID != 0 && currentTeacherID != rows[i].TeacherID {
		...
		// fill display slice
		if displayRow[0] == "" {
			displayRow[0] = rows[i].TeacherName
		}
		if displayRow[1] == "" {
			displayRow[1] = time.Time(rows[i].StartTime).Format("15:04")
		}
		displayRow[int(rows[i].DayOfWeek)+1] = fmt.Sprintf("%d-(%s)-%s",
			rows[i].Grade, rows[i].ClassName,
			rows[i].SubjectName,
		)

		currentTeacherID = rows[i].TeacherID
